...
RSpec.describe BookingsController, type: :controller do
  ...
  context "POST #create" do 
    it "should create a valid booking with all attributes" do 
      ticket = FactoryGirl.create(:ticket)
      post :create,  params: { booking: { date_of_movie: Faker::Date.forward(30), no_of_persons: Faker::Number.number(2), amount: Faker::Number.decimal(5, 2), seats_alloted: Faker::Number.number(2), ticket_id: ticket.id}}, format: :json
      response.should have_http_status(:ok)
    end
  end 
  ...
  context "PUT #update" do
    it "should update the booking with valid attributes" do
      ticket = FactoryGirl.create(:ticket)
			booking = FactoryGirl.create(:booking ,ticket_id: ticket.id)
      put :update, params: { id:booking.id, booking: { date_of_movie: booking.date_of_movie, no_of_persons: booking.no_of_persons, amount: booking.amount, seats_alloted: booking.seats_alloted}}, format: :json
  ...
  context 'POST #create' do
    it 'should not create a booking with invalid attributes' do
      ticket = FactoryGirl.create(:ticket)
      post :create,  params: {booking: {date_of_movie: Faker::Date.forward(30)}},format: :json
      response.should have_http_status(:unprocessable_entity)
    end

    it 'should not create a booking with nil values' do
      booking= FactoryGirl.create(:booking)
      post :create, params: {booking: {date_of_movie: nil}}, format: :json
      response.should have_http_status(:unprocessable_entity)
    end
  end
  ...
  context 'PUT #update' do
    it 'should not update booking with invalid id' do
      ticket = FactoryGirl.create(:ticket)
			booking = FactoryGirl.create(:booking ,ticket_id: ticket.id)
      put :update, params: {id:511,booking: {date_of_movie: "12/12/2018"}}, format: :json
      response.should have_http_status(:not_found)
    end
  end 
